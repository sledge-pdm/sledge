...
const COMMON_BROWSER_ARGS: &str =
  "--enable-features=msWebView2EnableDraggableRegions --disable-features=ElasticOverscroll,msWebOOUI,msPdfOOUI,msSmartScreenProtection --disable-extensions --disable-plugins --disable-dev-shm-usage";
...
fn next_editor_label(app: &AppHandle) -> String {
  loop {
    let label = format!("editor-{}", Uuid::new_v4().simple().to_string()[..8].to_owned());
...
#[tauri::command]
pub async fn open_window(app: AppHandle, kind: SledgeWindowKind, options: Option<WindowOpenOptions>) -> Result<(), String> {
  ...
  let path_script = format!("window.__PATH__={};", serde_json::to_string(&open_path.unwrap_or_default()).unwrap_or_default());
  ...
  match kind {
    ...
    SledgeWindowKind::About => {
      builder = builder.title("about.").inner_size(380.0, 260.0).resizable(false).closable(true).minimizable(false).maximizable(false);
    }
    SledgeWindowKind::Settings => {
      builder = builder.title("settings.").inner_size(600.0, 400.0).resizable(false).closable(true).minimizable(false).maximizable(false);
    }
  }
