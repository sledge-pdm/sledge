    ...
    <>
      ...
      <div class={wikiContentRoot}>
        ...
        <div class={wikiMarkdown}>
          <h2>About</h2>
          <p>Import creates a new project from an image when you open something that is not a <code>.sledge</code> file.</p>
          <ul>
            ...
            <li>The project is marked as opened “as image E(openAs = <code>image</code>), so saving will ask for a destination.</li>
          </ul>

          <h2>From file</h2>
          <ol>
            <li>Choose a PNG / JPEG / WEBP file (drag-in or open from the explorer section).</li>
            <li>sledge clears the current selection, resizes the canvas to the file, and registers the pixel data on a new layer named after the file.</li>
            <li>The layer buffers and previews are regenerated immediately.</li>
          </ol>
          <p>If the path pointed to a <code>.sledge</code> file, sledge switches to “Open project Einstead of importing.</p>
