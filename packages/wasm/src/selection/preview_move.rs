            ...
            if let Some(pixel) = selected_pixels[idx] {
                ...
                if new_x >= 0 && new_x < w && new_y >= 0 && new_y < h {
                    ...
                    if new_pixel_start + 3 < result.len() {
                        ...
                        let src_alpha = pixel[3];
                        let dst_alpha = result[new_pixel_start + 3];
                        ...
                        } else if src_alpha == 255 && dst_alpha == 255 {
                            // 両方とも完全不透明な場合は移動元で上書き（高速処理）
                            result[new_pixel_start] = pixel[0];
                            result[new_pixel_start + 1] = pixel[1];
                            result[new_pixel_start + 2] = pixel[2];
                            result[new_pixel_start + 3] = pixel[3];
                        } else {
                            // 半透明が含まれる場合のみアルファブレンディング
                            let src_alpha_f = src_alpha as f32 / 255.0;
                            let dst_alpha_f = dst_alpha as f32 / 255.0;
                            let inv_src_alpha = 1.0 - src_alpha_f;

                            result[new_pixel_start] = (pixel[0] as f32 * src_alpha_f
                                + result[new_pixel_start] as f32 * inv_src_alpha)
                                as u8;
                            result[new_pixel_start + 1] = (pixel[1] as f32 * src_alpha_f
                                + result[new_pixel_start + 1] as f32 * inv_src_alpha)
                                as u8;
                            result[new_pixel_start + 2] = (pixel[2] as f32 * src_alpha_f
                                + result[new_pixel_start + 2] as f32 * inv_src_alpha)
                                as u8;
                            result[new_pixel_start + 3] =
                                ((src_alpha_f + dst_alpha_f * inv_src_alpha) * 255.0) as u8;
                        }
                    }
                }
            }
