...
import { allLayers, canvasStore, imageStore, layerStore, setCanvasStore, updateDSL } from "~/stores/Store";
...
const CanvasSettings: Component<{}> = (props) => {
    ...
    return <div class={styles.root}>
        ...
        <form class={styles.size_form} onSubmit={(e) => changeCanvasSize(e)}>
            <div>
                <p>width</p>
                <input class={styles.size_input}
                    type="number"
                    name="width"
                    onChange={(e) => setWidth(Number(e.target.value))}
                    value={width()} min={0} max={1200} required />
            </div>
            <div>
                <p>width</p>
                <input class={styles.size_input}
                    ...
                    value={height()} min={0} max={1200} required />
            </div>
            <button class={styles.button} type="submit">change</button>
        </form>

        <button class={styles.button} onClick={resetAllLayers}>RESET ALL LAYERS</button>
    </div>;
};
