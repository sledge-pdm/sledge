...
export class ProjectHistoryController {
  ...
  deserialize(serialized: SerializedHistoryAction): BaseHistoryAction | undefined {
    try {
      const props = serialized.props as any;
      switch (serialized.type) {
        case 'canvas_size':
          return new CanvasSizeHistoryAction(props);
        case 'color':
          return new ColorHistoryAction(props);
        case 'convert_selection':
          return new ConvertSelectionHistoryAction(props);
        case 'image_pool':
          return new ImagePoolHistoryAction(props);
        case 'layer_buffer':
          return new LayerHistoryAction(props);
        case 'layer_list':
          return new LayerListHistoryAction(props);
        case 'layer_list_cut_paste':
          return new LayerListCutPasteHistoryAction(props);
        case 'layer_list_reorder':
          return new LayerListReorderHistoryAction(props);
        case 'layer_merge':
          return new LayerMergeHistoryAction(props);
        case 'layer_props':
          return new LayerPropsHistoryAction(props);
      }
      return undefined;
    } catch (e) {
