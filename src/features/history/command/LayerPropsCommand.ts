...
interface LayerPropsCommandContext extends CommandContext {
  layerId: string;
  oldLayerProps?: Omit<Layer, 'id'>;
  newLayerProps?: Omit<Layer, 'id'>;
}
...
export class LayerPropsCommand extends HistoryCommand {
  ...
  registerBefore(layer?: Layer) {
    const target = layer ?? this.getLayerById(this.layerId);
    if (target) {
  ...
  private getLayerById(layerId: string): Layer | undefined {
    return projectStore.layers.layers.find((l) => l.id === layerId);
  }

  getContext(): LayerPropsCommandContext {
    return { layerId: this.layerId, oldLayerProps: this.oldLayerProps, newLayerProps: this.newLayerProps };
  }
}
